...
dis_template = Path(base_path, 'dis_template.py_tmpl').read_file()
...
def main():
    def gen_tests():
        for Tx, group_x_dir in enumerate(test_path.walk(filter=DIRS), 1):
            print("Gen Test: {}".format(Tx))
            group = []
            for tx, test_x_file in enumerate(group_x_dir.walk(filter=FILES), 1):
                print("    Sub Test: {}".format(tx))
                file_content = test_x_file.read_file()

                dis_test_content = dis_template.format(file_content)

                dis_test_file = tmp.child('__pfw_{}_{}'.format(group_x_dir.name, test_x_file.name))
